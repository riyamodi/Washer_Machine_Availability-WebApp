...
def actual_text(m, text_body):
	...
	message = client.sms.messages.create(body="%s %s at %s %s" %(m.type, text_body, m.location.dorm, m.location.floor), to="+15107898157", from_="+15104021338")
...
def pre_text(l_id):
	...
	while True:
		...
		#so need to get location_id when someone preses the "send me a text" button
		#on a page and pass it into this function as l_id
