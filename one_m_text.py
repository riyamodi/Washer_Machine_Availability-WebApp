...
def actual_text(machine, text_body):
	...
	message = client.sms.messages.create(body="%s %s at %s %s" %(machine.type, text_body, machine.location.dorm, machine.location.floor), to="+15107898157", from_="+15104021338")
...
def pre_text(m_id):
	...
	while True:
		...
		load = model.session.query(model.Load).filter_by(machine_id=m_id).all()
		print "load: ", load
		load = load[-1]
